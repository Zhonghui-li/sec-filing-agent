...
def guardrail_check(answer: str, tools_used: List[str], trace: List[Dict] = None):
    ...
    # DETECTION, not a block — and that is a decision, not an omission. The harm here was never the
    # arithmetic: compute_formula ATTACHED A FILING ACCESSION to a figure no filing contains. That
    # is fixed at the tool, which now computes the same result, labels it an estimate from the
    # caller's assumption, and carries no citation. Asked again, the model either says so in the
    # answer or declines outright. Replacing an answer that already tells the reader the number is
    # not reported is the disproportionate intervention note 29 was about. Genuinely untraceable
    # figures are still blocked above.
    #
    # Residual, worth a narrower fix than blocking: the model may still append a stray accession of
    # its own next to the estimate.
